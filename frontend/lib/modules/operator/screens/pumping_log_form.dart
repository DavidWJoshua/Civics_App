...
class _PumpingLogFormState extends State<PumpingLogForm> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Failed to submit log. Please try again."), backgroundColor: Colors.red));
    } finally {
    ...
    switch (freq) {
      case 'daily':
        return [
          ...
          _section("Remarks"),
          _text("Daily Remark", "remark", maxLength: 1000),
      ...
      case 'weekly':
        return [
          ...
          _section("Remarks"),
          _text("Weekly Remark", "remark", maxLength: 1000),
          _imagePicker("Photo Evidence", "photo_url"),
        ];
      case 'monthly':
        return [
          ...
          _text("Preventive Action Taken", "preventive_action", maxLength: 1000),
          _section("Remarks"),
          _text("Monthly Remark", "remark", maxLength: 1000),
          _imagePicker("Photo Evidence", "photo_url"),
        ];
      case 'yearly':
        return [
          ...
          _section("Remarks"),
          _text("Yearly Remark", "remark", maxLength: 1000),
  ...
  Widget _text(String label, String key, {bool isNumber = false, bool isInt = false, bool required = false, int? maxLength}) {
    return Padding(
