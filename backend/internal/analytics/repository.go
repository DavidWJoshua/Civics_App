...
// allowedBreakdownFields is the exhaustive allowlist for GetComplaintBreakdown.
// Only these exact column names may appear in a GROUP BY query.
var allowedBreakdownFields = map[string]string{
	"severity": "SELECT severity, COUNT(*) FROM complaints GROUP BY severity",
	"category": "SELECT category, COUNT(*) FROM complaints GROUP BY category",
}
...
func (r *Repository) GetComplaintTrends(ctx context.Context, days int) ([]TimePoint, error) {
	...
	// Use a parameterized query — never interpolate values into SQL strings.
	// PostgreSQL supports multiplying an interval by a parameter directly.
...
func (r *Repository) GetComplaintBreakdown(ctx context.Context, field string) (map[string]int64, error) {
	// Use a static query map — no dynamic SQL construction.
	// Only the two allowed fields have entries; anything else returns an error.
	query, ok := allowedBreakdownFields[field]
	if !ok {
