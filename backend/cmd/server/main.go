...
func main() {
	...
	api := r.Group("/api")
	// Global rate limit: 120 requests/min per IP across all API routes.
	api.Use(middleware.RateLimitMiddleware(120, time.Minute))
	// Global body size cap: 1 MB for all JSON endpoints (prevents resource exhaustion).
	// Multipart upload routes enforce their own ParseMultipartForm(5MB) limit.
	api.Use(middleware.MaxBodySize(1 << 20))

	// PUBLIC ROUTES — stricter rate limit (10 req/min) to block OTP brute-force.
	authRoutes := api.Group("/auth")
	authRoutes.Use(middleware.RateLimitMiddleware(10, time.Minute))
	auth.RegisterRoutes(authRoutes, authHandler)
	authRoutes.POST("/citizen/verify-otp", authHandler.VerifyOTP)

	// Protected auth actions (require valid JWT)
